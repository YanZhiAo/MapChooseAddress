...
@interface LongPressMapChooseVC ()<MAMapViewDelegate,AMapSearchDelegate,AMapLocationManagerDelegate,UITextFieldDelegate,MLKMenuPopoverDelegate,UIGestureRecognizerDelegate>
...
@implementation LongPressMapChooseVC
...
- (void)viewDidLoad {
    ...
    lpress.minimumPressDuration = 2.0;//按1.0秒响应longPress方法
    lpress.delegate = self;
    lpress.allowableMovement = 50.0f;
    [self.mapView addGestureRecognizer:lpress];//m_mapView
...
-(void)menuPopover:(MLKMenuPopover *)menuPopover didSelectMenuItemAtIndex:(NSInteger)selectedIndex
{
    ...
    self.regeocode = poiResult.address;
    
    self.labAddress.text = poiResult.address;
...
-(void)BackPopController:(UIButton *)btn
{
    ...
    if ([self.delegate respondsToSelector:@selector(LongPressMapDidClickGeoPoint:regeocode:Province:City:)]) {
        [self.delegate LongPressMapDidClickGeoPoint:self.location regeocode:self.regeocode Province:self.Province City:self.City];
    }
...
- (void)onReGeocodeSearchDone:(AMapReGeocodeSearchRequest *)request response:(AMapReGeocodeSearchResponse *)response
{
    if (response.regeocode != nil)
    {
        ...
        //[self.mapView setSelectedAnnotations:@[self.pointAnnotation]];
        [self.mapView addAnnotation:self.pointAnnotation];
